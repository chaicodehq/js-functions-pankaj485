...
export function createDialogueWriter(genre) {
  ...
  const generateDialogue = (hero, villan) => {
    if (!hero || !villan || hero.trim() === "" || villan.trim() === "") {
      return "...";
    }

    return templateFn(hero, villan);
  };
...
export function createTicketPricer(basePrice) {
  ...
  const calculatePrice = (seatType, isWeekend = false) => {
    const multiplier = seatMultipliers[seatType.toLowerCase()];
  ...
  const calculateRating = (scores) => {
    ...
    Object.entries(scores).forEach(([key, score]) => {
      if (weights.hasOwnProperty(key)) {
        weightedAverage += score * weights[key];
      }
    });
